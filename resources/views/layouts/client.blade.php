...
<body class="font-sans antialiased bg-[#eae6df]" style="font-family: 'Google Sans', ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
    <div class="min-h-screen flex">
        <!-- Sidebar -->
        <aside class="hidden w-56 fixed h-full z-10 transition-all duration-300 border-r bg-slate-950 text-white border-slate-800 shadow-[0_25px_60px_rgba(15,23,42,0.45)] rounded-r-[32px]">
            <!-- Logo -->
            ...
            <nav class="mt-2 px-2 space-y-2">
                <a href="{{ route('client.dashboard') }}"
                    title="Dashboard" class="group relative flex items-center gap-3 px-3 py-2.5 rounded-2xl transition-colors duration-200 {{ request()->routeIs('client.dashboard') ? 'bg-white/15 text-white shadow-[0_12px_30px_rgba(15,23,42,0.35)]' : 'text-white/70 hover:bg-white/10 hover:text-white' }}">
                ...
                <a href="{{ route('docs.index') }}"
                    title="Documentation" class="group flex items-center gap-3 px-3 py-2.5 rounded-2xl transition-colors duration-200 text-white/70 hover:bg-white/10 hover:text-white">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('pricing') }}"
                    title="Pricing" class="group flex items-center gap-3 px-3 py-2.5 rounded-2xl transition-colors duration-200 text-white/70 hover:bg-white/10 hover:text-white">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('contact') }}"
                    title="Contact" class="group flex items-center gap-3 px-3 py-2.5 rounded-2xl transition-colors duration-200 text-white/70 hover:bg-white/10 hover:text-white">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('client.settings.profile') }}"
                    title="Profile Settings" class="group flex items-center gap-3 px-3 py-2.5 rounded-2xl transition-colors duration-200 {{ request()->routeIs('client.settings.profile') ? 'bg-white/15 text-white shadow-[0_12px_30px_rgba(15,23,42,0.35)]' : 'text-white/70 hover:bg-white/10 hover:text-white' }}">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <!-- Main Content -->
        <div class="flex-1 transition-all duration-300 ml-0">
            <header class="relative h-20 fixed top-0 right-0 left-0 z-10 transition-all duration-300">
                <div class="h-full bg-[#0b0b0c] text-white shadow-[0_20px_40px_rgba(0,0,0,0.35)]">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="text-sm text-white/70">Client Dashboard</div>
                    </div>
                    <div class="text-right">
                        <p class="text-sm font-semibold text-white">{{ Auth::guard('web')->user()->name }}</p>
                        <p class="text-xs text-white/60">{{ Auth::guard('web')->user()->email }}</p>
                    </div>
                    </div>
                </div>
            </header>
