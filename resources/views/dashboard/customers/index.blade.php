<x-app-layout>
    <div class="p-8">
        <div class="mb-8">
            ...
            <p class="text-gray-600">View and manage your customers based on transaction history</p>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($customers as $customer)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $customer->customer_email }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $customer->customer_phone ?? '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ number_format($customer->total_transactions) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">Rp {{ number_format($customer->total_amount, 0, ',', '.') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ \Carbon\Carbon::parse($customer->last_transaction_at)->format('M d, Y H:i') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <a href="{{ route('dashboard.customers.show', base64_encode($customer->customer_email)) }}" class="text-purple-600 hover:text-purple-700 font-medium">View</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-8 text-center text-sm text-gray-500">No customers found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
